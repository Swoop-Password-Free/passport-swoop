...
function Strategy(options, verify){
  ...
  options.state = true;
  options.scope = 'email';
  options.authorizationURL = options.authorizeURL || 'https://auth.swoop.email/oauth2/authorize';
  options.tokenURL = options.tokenURL || 'https://auth.swoop.email/oauth2/token';
  this._userProfileURL = options.userProfileURL || 'https://auth.swoop.email/oauth2/profile';
 ...
 */
 OAuth2Strategy.prototype.authorizationParams = function(options) {
   ...
   if(options && options.property_meta) {
     let token = jwt.sign(options.property_meta, this._oauth2._clientSecret);
     params['property_meta']=token;
   }
